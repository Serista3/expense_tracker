...
<template>
    <div @click.self="emit('closeNav')" :class="{'visible' : activeNav, 'invisible': !activeNav}" class="nav__bg backdrop-blur-[1px] bg-[rgba(0,0,0,0.3)] fixed top-0 left-0 w-full h-screen z-10">
        <nav :class="{'translate-x-[0]' : activeNav, '-translate-x-[100%]' : !activeNav}" class="nav__sidebar pt-30 pb-40 pl-22 pr-32 max-w-[35rem] w-full h-screen border-r border-gray-300 bg-light dark:bg-dark dark:text-light fixed top-0 left-0 z-20 transition-all">
            <PhX @click.self="emit('closeNav')" :size="18" class="close-nav cursor-pointer absolute text-gray-400 top-8 right-8 hover:text-dark transition-all" />
            <UserProfile />
